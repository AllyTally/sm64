...
u32 determine_interaction(struct MarioState *m, struct Object *o) {
    ...
    if (action & ACT_FLAG_ATTACKING) {
        ...
        } else if (action == ACT_GROUND_POUND || action == ACT_TWIRLING || action == ACT_WATER_GROUND_POUND) {
            if (m->vel[1] < 0.0f) {
                interaction = INT_GROUND_POUND_OR_TWIRL;
            }
        } else if (action == ACT_GROUND_POUND_LAND || action == ACT_TWIRL_LAND || action == ACT_WATER_GROUND_POUND) {
